  ...
  async function sync() {
    ...
    for (const op of queue) {
      try {
        ...
        } else if (op.entityType === 'return' && op.operation === 'create') {
          await request('/rest/v1/rpc/record_sale_return', 'POST', token, { p_business_id: state.business.id, p_branch_id: null, p_sale_id: p.saleId, p_customer_id: p.customerId || null, p_return_number: `RET-${p.id}`, p_reason: p.reason || null, p_items: (p.items || []).map((i) => ({ product_id: i.productId, quantity: Number(i.quantity), unit_price: Number(i.unitPrice) })) });
        } else if (op.entityType === 'stock_adjustment' && op.operation === 'create') {
          await request('/rest/v1/rpc/record_stock_adjustment', 'POST', token, { p_business_id: state.business.id, p_branch_id: null, p_product_id: p.productId, p_variant_id: null, p_counted_quantity: Number(p.countedQuantity), p_reason: p.reason || 'جرد دوري', p_count_number: `COUNT-${p.id}` });
        } else { continue; }

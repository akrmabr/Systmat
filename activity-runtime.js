...
(() => {
  ...
  const renderFields = async () => {
    ...
    if (!form || form.parentElement.querySelector('.najah-extra-fields')) return;
    ...
    wrapper.innerHTML = `<div class="najah-extra-heading"><strong>خصائص ${config()?.label || 'النشاط'}</strong><span>تُحفظ داخل بيانات المنتج وتظهر في البحث والمزامنة.</span></div>`;
